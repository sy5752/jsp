...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for(Cookie cookie : cookies) {
			logger.debug("cookie.getName() : {}", cookie.getName(), cookie.getValue());
		...
		// 로그인 프로세스
		// db에 저장된 사용자 정보와 일치하는지 검증해야하나, db연동이 아직 준비되지 않은
		// 관계로 userid가 brown일 때 비밀번호가 brownpass인 경우에 한해 로그인이 성공되었다고 판단
		// 그 외 경우는 로그인 실패로 간주
		...
		// 로그인 실패시 : login.jsp redirect(문법적으로 배우기 위해, 올바른 상황에 대해서는 추후 다시)
		...
		// 3. 2번 비교 사항이 true일 때 webapp/main.jsp로 forward
